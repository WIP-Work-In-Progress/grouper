...
const Participant = {
  ...
  getByRegistrationId: (registrationId) => {
    const stmt = db.prepare("SELECT * FROM participants WHERE registration_id = ?");
    const participants = stmt.all(registrationId);
    return camelCaseKeys(participants);
  },
  getByLastName: (name) => {
    const stmt = db.prepare("SELECT * FROM participants WHERE last_name LIKE ?");
    const participants = stmt.all("%" + name + "%");
    return camelCaseKeys(participants);
  },
  getByFirstName: (name) => {
    const stmt = db.prepare("SELECT * FROM participants WHERE first_name LIKE ?");
    const participants = stmt.all("%" + name + "%");
    return camelCaseKeys(participants);
  },
  getByAlbumNumber: (album) => {
    const stmt = db.prepare("SELECT * FROM participants WHERE album_number = ?");
  ...
  create: (firstName,lastName, registrationId,albumNumber,email,token,admin) => {
    const stmt = db.prepare(
      "INSERT INTO participants (first_name, last_name, registration_id, album_number, email, token, is_admin) VALUES (?, ?, ?, ?, ?, ?, ?)"
    );
    const result = stmt.run(firstName, lastName, registrationId , albumNumber, email, token, admin);
  ...
  update: (id, firstName , lastName, registrationId, albumNumber ,email,token,admin) => {
    const stmt = db.prepare(
      "UPDATE participants SET first_name = ?, last_name = ?, registration_id = ?, album_number = ?, email = ?, token = ?, is_admin = ?  WHERE id = ?"
    );
    const result = stmt.run(firstName, lastName, registrationId , albumNumber, email, token, admin, id);
